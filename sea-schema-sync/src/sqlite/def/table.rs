...
impl From<RusqliteRow> for TableDef {
...
impl TableDef {
    ...
    pub fn pk_is_autoincrement(&mut self, conn: &impl Connection) -> DiscoveryResult<&mut Self> {
        ...
        if !conn.query_all(check_autoincrement)?.is_empty() {
    ...
    pub fn get_constraints(&mut self, conn: &impl Connection) -> DiscoveryResult<()> {
        ...
        let partial_index_info_rows = conn.query_all_raw(index_query)?;
        ...
        for partial_index in partial_indexes {
            let partial_index_column: IndexedColumns =
                self.get_single_indexinfo(conn, &partial_index.name)?;
    ...
    /// Note that this does not get the column name mapped by the index.
    /// To get the column name mapped by the index, the `self.get_single_indexinfo` method is invoked
    pub fn get_indexes(&mut self, conn: &impl Connection) -> DiscoveryResult<()> {
        let mut index_query = String::default();
        index_query.push_str("PRAGMA index_list('");
        index_query.push_str(&self.name);
        index_query.push_str("')");
    ...
    pub fn get_foreign_keys(&mut self, conn: &impl Connection) -> DiscoveryResult<&mut Self> {
        let mut index_query = String::default();
        index_query.push_str("PRAGMA foreign_key_list('");
        index_query.push_str(&self.name);
        index_query.push_str("')");

        let index_info_rows = conn.query_all_raw(index_query)?;
    ...
    pub fn get_column_info(&mut self, conn: &impl Connection) -> DiscoveryResult<&TableDef> {
        let mut index_query = String::default();
    ...
    pub(crate) fn get_single_indexinfo(
        &mut self,
        ...
        let index_info = conn.query_all(index_query)?;
        ...
        let index_column_info_rows = conn.query_all_raw(index_column_query)?;
