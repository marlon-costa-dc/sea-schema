...
use sea_schema::{
    rusqlite_types::{connect_sqlite, execute_sqlite},
    sqlite::{
        def::TableDef,
...
fn test_001() -> DiscoveryResult<()> {
    let url = std::env::var("DATABASE_URL_LIVE").unwrap_or_else(|_| "sqlite::memory:".to_owned());

    let pool = connect_sqlite(&url).unwrap();

    //DROP TABLES to ensure all tests pass
    execute_sqlite(&pool, "DROP TABLE IF EXISTS Programming_Langs").unwrap();

    execute_sqlite(&pool, "DROP TABLE IF EXISTS suppliers").unwrap();

    execute_sqlite(&pool, "DROP TABLE IF EXISTS supplier_groups").unwrap();
    ...
    execute_sqlite(&pool, &create_table.to_string(SqliteQueryBuilder)).unwrap();

    execute_sqlite(&pool, &create_table_inventors.to_string(SqliteQueryBuilder)).unwrap();

    execute_sqlite(&pool, &insert_into_table.to_string(SqliteQueryBuilder)).unwrap();
    ...
    execute_sqlite(&pool, &table_create_suppliers.to_string(SqliteQueryBuilder)).unwrap();
    ...
    execute_sqlite(&pool, &insert_into_suppliers.to_string(SqliteQueryBuilder)).unwrap();

    execute_sqlite(&pool, &create_index.to_string(SqliteQueryBuilder)).unwrap();

    let schema = SchemaDiscovery::new(pool).discover()?;
...
fn test_002() -> DiscoveryResult<()> {
    let url = std::env::var("DATABASE_URL_LIVE").unwrap_or_else(|_| "sqlite::memory:".to_owned());

    let connection = connect_sqlite(&url).unwrap();
    ...
    for tbl_create_stmt in tbl_create_stmts.iter() {
        ...
        execute_sqlite(&connection, &sql).unwrap();
    }
